    ...
    try {
        ...
        for (const item of items) {
            ...
            await query(`
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            `, [uuidv4(), orderId, item.productId, item.quantity, item.price, lineTotal]);
...
export async function getOrderDetails(orderNumber: string) {
    try {
        ...
        const items = await query(`
            SELECT si.quantity, si.unit_price, si.line_total, p.name, p.sku
            FROM sales_items si
...
export async function syncPosOrder(orderData: any) {
    try {
        ...
        const result = await createSalesOrder({
            ...
            paymentMethod: 'CASH', // Default or add to OrderDoc
