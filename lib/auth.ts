...
export function checkRouteAccess(user: User | null, path: string): boolean {
    ...
    // Super User bypass (optional, but good for safety)
    if (user.role === 'super_user') return true;

    if (path.startsWith('/paths/POS')) return user.permissions.includes('pos.access');
    if (path.startsWith('/paths/Ticket')) return user.permissions.includes('tickets.manage');
    if (path.startsWith('/paths/admin/inventory')) return user.permissions.includes('inventory.view');
    if (path.startsWith('/paths/HR')) return user.permissions.includes('hr.view');
    if (path.startsWith('/paths/admin/users')) return user.permissions.includes('users.manage');
    if (path.startsWith('/paths/admin')) return user.permissions.includes('admin.view');
